...
//		printk("memblock %i: base=%llx size=%llx\n", i, r->base, r->size);
	...
	// Located just after the core and host sections
	pool_start = el2_data->page_pool_start + STAGE2_CORE_PAGES_SIZE + STAGE2_HOST_POOL_SIZE;

	// GOAL: Remove this. For now it's only useful in a VM as a placeholder until
	// dynamic allocated regions are added as parameters to the register_kvm() function
	// This sets the page_pool_starts for all VMs in a fixed manner.
	// This should be moved to where the VM at that index actually ends up getting used,
	// which is register_kvm().
